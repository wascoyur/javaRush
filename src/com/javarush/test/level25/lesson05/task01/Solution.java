...
public class Solution {
    public static void processThreads(Thread... threads) {
        ...
        for (Thread x: threads) {
                switch (x.getState()) {
                    case NEW: x.start(); break;
                    case WAITING: x.interrupt(); break;
                    case TIMED_WAITING: x.interrupt();break;
                    case BLOCKED: x.interrupt();break;
                    case RUNNABLE: x.isInterrupted();break;
                    case TERMINATED: System.out.println(x.getPriority());break;
            }
        }
    }
